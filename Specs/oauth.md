...
Add an OAuth 2.0 + PKCE login flow to `lean auth login` so users can
authenticate via Linear's hosted authorization page (the same UX you
get from `gh auth login`) instead of pasting a long-lived Personal API
Key. The existing API-key path stays for headless / CI use, and
becomes the documented path for non-interactive automation.

See `Specs/oauth-research.md` for the underlying capability matrix this
spec is built on.
